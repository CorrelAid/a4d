    ...
    return pl.DataFrame(
        {
            ...
            "patient_id": [f"KH_KB{i:03d}" for i in range(len(mg))],
...
class TestResolveGlucoseUnits:
    def test_a_wholly_mmol_column_is_moved_to_its_mmol_sibling(self):
        ...
        out = resolve_glucose_units(df)

        assert out["fbg_updated_mmol"].to_list()[0] == 8.0
        assert out["fbg_updated_mg"].to_list()[0] == 8.0 * MMOL_TO_MG_FACTOR

    def test_a_swapped_column_is_reported_once_not_once_per_row(self, collector):
        """The defect is the column's label, so the source fix is file-level."""
        df = _frame([8.0, 9.4, 12.1, 7.2, 15.0, 6.6, 19.3, 11.0, 5.5, 22.0])

        resolve_glucose_units(df)

        swapped = [e for e in collector.findings if e.error_code == "glucose_unit_swapped"]
    ...
    def test_a_stray_low_reading_in_a_clean_column_is_flagged_not_converted(self, collector):
        ...
        suspect = [e for e in collector.findings if e.error_code == "glucose_unit_suspect"]
        assert len(suspect) == 1
        assert suspect[0].patient_id == "KH_KB009"

    def test_a_swapped_column_raises_no_row_level_suspicion(self, collector):
        df = _frame([8.0, 9.4, 12.1, 7.2, 15.0, 6.6, 19.3, 11.0, 5.5, 22.0])

        resolve_glucose_units(df)

        assert not [e for e in collector.findings if e.error_code == "glucose_unit_suspect"]
    ...
    def test_missing_columns_are_tolerated(self):
        df = pl.DataFrame({"file_name": ["t.xlsx"], "patient_id": ["KH_KB001"]})

        assert resolve_glucose_units(df).equals(df)
