...
def test_normalize_patient_id_strips_transfer_suffix() -> None:
    df = pl.DataFrame(
        {"patient_id": ["MY_SM003_SB", "LA-MH093_LF", "TH_NK001", "SOLO"]},
        schema={"patient_id": pl.Utf8},
    )
    out = df.with_columns(normalize_patient_id(pl.col("patient_id")).alias("n"))
    assert out["n"].to_list() == ["MY_SM003", "LA_MH093", "TH_NK001", "SOLO"]
...
def test_missing_patient_does_not_fire_for_transferred_id() -> None:
    ...
    cleaned = _make_clean(
        [
            {
                "clinic_id": "CDA",
                "patient_id": "MY_SM003",  # cleaner stripped suffix
                "tracker_year": 2024,
                "tracker_month": 1,
                "file_name": "f",
                "weight": 70.0,
...
def test_missing_patient_fires_when_truly_absent() -> None:
    raw = _make_raw(
        [
            ...
            {
                "clinic_id": "CDA",
                "patient_id": "MY_SM999",  # absent in cleaned
                "tracker_year": 2024,
                "tracker_month": 1,
                "file_name": "f",
                "weight": "60",
    ...
    assert any("MY_SM999" in m and "MISSING_ROW" in m for m in msgs)
