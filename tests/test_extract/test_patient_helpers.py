...
class TestReadPatientRows:
    ...
    def test_keeps_a_numbered_row(self):
        wb, ws = self._sheet([[1, "VN_QC001", "VN_QC001", "Y"]])
    ...
    def test_keeps_an_unnumbered_row_that_carries_data(self):
        ...
        wb, ws = self._sheet([[None, "LA-QA088", "LA-QA088", "Y", "2024-06-19"]])
    ...
    def test_drops_an_unnumbered_row_that_only_repeats_its_own_identifier(self):
        ...
        wb, ws = self._sheet([[None, "VN_QC052", "VN_QC052", None, None]])
    ...
    def test_stops_at_the_first_completely_empty_row(self):
        wb, ws = self._sheet(
            [[1, "VN_QC001", "VN_QC001"], [None, None, None], [2, "VN_QC002", "VN_QC002"]]
        )
    ...
    def test_skips_a_row_with_neither_number_nor_identifier(self):
        wb, ws = self._sheet([[None, None, "stray note"], [1, "VN_QC001", "VN_QC001"]])
    ...
    def test_recovers_a_bare_year_typed_into_a_date_formatted_cell(self):
        """2019 Yangon Children's Patient List!F, MM_QE005: the D.O.B. cell
        ...
        wb, ws = self._sheet([[1, "MM_QE005", "MM_QE005", None]])
...
class TestFindDroppedDataColumns:
    ...
    def test_ignores_row_counter(self):
        ...
        headers = [None, "ID"]
        data = [("1", "KH_QF001"), ("2", "KH_QF002"), ("3.0", "KH_QF003")]
    ...
    def test_reports_mixed_numeric_and_text_column(self):
        ...
        headers = [None, "ID"]
        data = [("7", "KH_QC001"), ("*Needs Meter", "KH_QC002")]
...
class TestReportDuplicatePatientsInSheet:
    ...
    def test_reports_a_patient_listed_twice(self, collector):
        df = pl.DataFrame({"patient_id": ["VN_QC002", "VN_QC003", "VN_QC002"]})
        ...
        assert row["patient_id"] == "VN_QC002"
    ...
    def test_two_spellings_of_one_id_are_one_patient(self, collector):
        """Surat Thani writes ``TH-QG029`` and ``TH_QG029`` on the same sheet.
        ...
        """
        df = pl.DataFrame({"patient_id": ["TH-QG029", "TH_QG029"]})

        report_duplicate_patients_in_sheet(df, "May26")

        row = collector.to_dataframe().row(0, named=True)
        assert row["patient_id"] == "TH_QG029"
        assert "TH-QG029" in row["original_value"]
        assert "TH_QG029" in row["original_value"]
