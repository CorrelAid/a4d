...
def test_create_table_writes_output_with_full_schema(tmp_path: Path):
    ...
    _cleaned_product_df([_base_row(), _base_row(product_released_to="MY_QI002")]).write_parquet(
        cleaned_file
    )
...
def test_create_table_preserves_original_released_to(tmp_path: Path):
    ...
    cleaned_file = tmp_path / "tracker_a_product_cleaned.parquet"
    _cleaned_product_df([_base_row(product_released_to="MY-QI001")]).write_parquet(cleaned_file)
    output_dir = tmp_path / "tables"
    ...
    assert df["product_released_to"].to_list() == ["MY_QI001"]
    assert df["orig_product_released_to"].to_list() == ["MY-QI001"]
...
def test_create_table_concatenates_multiple_files(tmp_path: Path):
    ...
    _cleaned_product_df(
        [
            ...
            _base_row(file_name="b.xlsx", product_released_to="MY_QI003"),
        ]
