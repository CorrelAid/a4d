...
Depends on [Does product-pipeline's test suite meet the same cell-by-cell rigor
as patient's?](01-product-pipeline-test-rigor.md) and [Retire the PDF/notebook
analysis docs for an automated, script-based report](02-documentation-strategy.md)
being resolved first — the user's stated order is product-pipeline readiness
before the merge, not merge-then-fix.
