...
def clean_patient_data(
    ...
    # "Undefined" sentinel: 2019 Kantha Bopha's KH_QD023 has a sex cell reading
    ...
    # the sheet covers, while a date of birth does not (LA_QA060's D.O.B. of
...
def _fix_age_from_dob(df: pl.DataFrame) -> pl.DataFrame:
    ...
    date of birth stays correct. LA_QA060's recovered D.O.B. of 2009-01-16
...
def _fix_t1d_diagnosis_age(df: pl.DataFrame) -> pl.DataFrame:
    ...
    Deriving only into the gap keeps both: MM_QA010's cell reads `11yr`, which
