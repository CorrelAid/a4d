...
def _is_r_static_join_misses_respelled_id(m: CellMismatch) -> bool:
    ...
    **Its one measured over-claim is 5 cells, and it is not fixable at this
    ...
    ``TH_QG029`` and ``TH-QG029`` in ``May26`` -- normalization folds the two
...
def _is_r_age_not_derived_without_dob(m: CellMismatch) -> bool:
    ...
    LA_QA060 as 13 in ``Jan23`` where Python publishes 14 -- that patient's
...
def _is_r_never_derives_diagnosis_age(m: CellMismatch) -> bool:
    ...
    **Where the two dates come from matters, though the reason above does
    ...
    patients ticket 58 recovered: 2024 Mahosot's LA_QA056 is null on all three
