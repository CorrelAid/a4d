...
def _is_r_extraction_gap(m: CellMismatch) -> bool:
    ...
    - ``recruitment_date`` (ticket 28): R's static "Patient List"
      ...
      Memorial Medical Center, patient PH_QM001, "Date of Recruitment" =
...
def _is_r_static_join_misses_respelled_id(m: CellMismatch) -> bool:
    ...
    the same workbook -- 2023/2024 Mahosot write ``LA-MH056`` against a Patient
    List entry of ``LA_MH056``, 2024 CDA ``KH-CD016``, 2026 Surat Thani
    ``TH-ST029`` -- the raw key cannot match, and every static column comes out
    ...
    holds a full row for ``LA_MH056`` (Vientiane Capital, M, D.O.B.
...
def _is_r_ifelse_na_propagation(m: CellMismatch) -> bool:
    ...
    Hospital_Jun_26, May26, PH_BG001 and PH_BG004): both pipelines hold the
...
def _is_python_reads_bare_year(m: CellMismatch) -> bool:
    ...
    MY_SW001) where its 2025 and 2026 workbooks write the bare year, so the
...
def _is_r_na_unite_padding(m: CellMismatch) -> bool:
    ...
    KH_KB023 has ``JAN`` in Kidney and nothing else): the sub-columns R pads are
...
def _is_python_recovers_glucose_r_rejected(m: CellMismatch) -> bool:
    ...
    Ticket 57, one cell: 2022 Penang MY_PN004 Dec22 writes "-" in the mmol
...
def _is_wide_format_fragment_truncated(m: CellMismatch) -> bool:
    ...
    "MM_MD019-2(Error-1),..."): the source cell's free-text notes contain
