#' @title Process raw patient data
#'
#' @description
#' This function reads the raw patient data from a parquet file, cleans the data,
#' and exports the cleaned data as a parquet file.
#'
#' @param paths list of paths to the input and output directories.
#' @param patient_file name of the parquet file with the raw patient data.
#' @param patient_file_name base name of the parquet file with the raw patient data.
#' @param output_root root directory of the output folder.
#'
#' @export
process_raw_patient_file <- function(paths, patient_file, patient_file_name, output_root) {
    patient_file_path <-
        file.path(paths$output_root, patient_file)
    ...
    # blood pressure is given as sys/dias value pair before 2024,
    ...
    # Since 2024 the tracker habe three columns for human insulin and two columns for analog insulin
    ...
    logWarn(
        log_to_json(
            message = "Extra columns in patient dynamic data: {values['extra_cols']}.",
            values = list(extra_cols = paste(extra_cols, collapse = ", ")),
            script = "script2",
            file = "script2_process_patient_data.R",
            functionName = "process_raw_patient_file",
    ...
    logWarn(
        log_to_json(
            message = "Missing columns in patient dynamic data: {values['missing_cols']}.",
            values = list(missing_cols = paste(missing_cols, collapse = ",")),
            script = "script2",
            file = "script2_process_patient_data.R",
            functionName = "process_raw_patient_file",
            warningCode = "invalid_tracker"
